...
taxonomy, the resolve stage and the `gbp`, `reviews` and `sitetech` collectors are built; the
remaining collectors, analyse and render are not.
...
`sitetech` ✅ → **`localrank` ←** → `speedtolead` → `aivis` → analyse + render → run on 10 real
businesses.
